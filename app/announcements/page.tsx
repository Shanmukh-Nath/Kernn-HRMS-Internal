...
export default function AnnouncementsPage() {
  ...
  // Deep linking: auto-open announcement if `?id=...` is in URL
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (targetId) {
        const found = announcements.find((a) => a.id === targetId || String(a._id) === targetId);
        if (found) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
          ...
          {announcements.map((a) => {
            ...
            return (
              <div
                ...
                className={`p-6 rounded-3xl bg-white border transition shadow-xs flex flex-col justify-between relative overflow-hidden cursor-pointer hover:shadow-md ${
                  a.isPinned
      ...
      {selectedAnnouncement && (
        ...
        >
          <div
            className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 sm:p-7 space-y-5 max-h-[90vh] overflow-y-auto border border-slate-200 animate-scaleUp"
          ...
          >
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {selectedAnnouncement.priority === 'URGENT' ? (
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-slate-100 text-slate-700">
                      {selectedAnnouncement.category?.replace('_', ' ') || 'Notice'}
                    </span>
                  )}
                  {selectedAnnouncement.targetDept && selectedAnnouncement.targetDept !== 'ALL' && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-50 text-blue-700 border border-blue-100">
                      Target: {selectedAnnouncement.targetDept}
                    </span>
                  )}
                </div>
            ...
            <div className="flex items-center justify-between text-xs text-slate-500 bg-slate-50 p-3 rounded-2xl border border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span>Published by <strong>{selectedAnnouncement.authorName}</strong></span>
              </div>
              <div className="flex items-center gap-1 font-mono text-[11px]">
                <Calendar className="w-3.5 h-3.5 text-slate-400" />
                <span>{selectedAnnouncement.publishedAt ? format(new Date(selectedAnnouncement.publishedAt), 'dd MMM yyyy') : '—'}</span>
              </div>
            </div>
                ...
                <div className="flex items-center gap-2 text-emerald-800">
                  ...
                  <span>
                    {selectedAnnouncement.isAcknowledged
                      ? 'You acknowledged this bulletin.'
                      : 'Acknowledgement required for this notice.'}
                  </span>
                </div>
                {!selectedAnnouncement.isAcknowledged && (
                  <button
                    onClick={async () => {
                      await handleAcknowledge(selectedAnnouncement.id);
                      setSelectedAnnouncement((prev: any) => prev ? { ...prev, isAcknowledged: true } : null);
                    }}
                    className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs transition shrink-0"
                  >
                    Acknowledge
                  </button>
                )}
